...
public class HdfsFsShel extends ProcessPlugin {

  private static final Pattern CONTAINER_ID_FORMAT = Pattern
      .compile("(org\\.apache\\.hadoop\\.fs\\.FsShell)");

  public HdfsFsShel(float rate) {
    super(rate);
  }
  ...
  public ContainerData apply(String cmdLine, int pid) {
    ...
    if (m.find()) {
      String clsId =
          String.format("FsShell_%d_%d", pid, System.currentTimeMillis());
